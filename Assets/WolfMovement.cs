...
public class WolfMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentDirection.x > 0f)
        {
            ...
            asc.ChangingAnimationState("WolfRight");
        }
        else if (currentDirection.x < 0f)
        {
            Debug.Log("Object is moving left.");
            asc.ChangingAnimationState("WolfLeft");
        }

        if (currentDirection.y > 0f)
        {
            Debug.Log("Object is moving up.");
            asc.ChangingAnimationState("WolfBack");
        }
        else if (currentDirection.y < 0f)
        {
            Debug.Log("WolfFoward");
            asc.ChangingAnimationState("WolfFoward");
